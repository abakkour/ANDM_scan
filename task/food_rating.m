...
screennum = min(Screen('Screens')); %select laptop screen
...
shuff_names=Shuffle(dir('../stim/foodchoice/*.jpg'));
Images = cell(1, length(shuff_names));
imx=0;
for i=1:length(shuff_names)
    imx=imx+2;
    Images{i}=imread(['../stim/foodchoice/',shuff_names(i).name]);
...
Eyelink('command', 'add_file_preamble_text ''Recorded by EyelinkToolbox Food Rating''');
